...
export function useTableUrlState(
  ...
  // Debounce the URL update for global filter
  useEffect(() => {
    if (!globalFilterEnabled || globalFilter === undefined) return

    // Don't update URL immediately if it matches the current search param
    const currentRaw = (search as SearchRecord)[globalFilterKey]
    const currentVal = typeof currentRaw === 'string' ? currentRaw : ''
    if (globalFilter === currentVal) return
    ...
    const timer = setTimeout(() => {
      navigate({
        search: (prev) => ({
          ...(prev as SearchRecord),
          [pageKey]: undefined,
          [globalFilterKey]: globalFilter || undefined,
        }),
      })
    }, 500)

    return () => clearTimeout(timer)
  }, [globalFilter, globalFilterEnabled, globalFilterKey, navigate, pageKey, search])
